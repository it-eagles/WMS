...
Public Class MultiIssuedWH
    ...
    Private Sub showunit2()
        Dim gg = From g In db.tblMasterCodes Where g.Type = "UNIT"
                  Select g.Type, g.Code
        Try
            ddlPickQTY_ConfirmIssue.DataSource = gg.ToList
            ddlPickQTY_ConfirmIssue.DataTextField = "Code"
            ddlPickQTY_ConfirmIssue.DataValueField = "Code"
            ddlPickQTY_ConfirmIssue.DataBind()

            If ddlPickQTY_ConfirmIssue.Items.Count > 1 Then
                ddlPickQTY_ConfirmIssue.Enabled = True
            Else
                ddlPickQTY_ConfirmIssue.Enabled = False
            End If
    ...
    Private Sub selectConsigneeCode()
        ...
        If String.IsNullOrEmpty(txtConsigneeCode0_IssueCon.Value.Trim) Then
            Ship_code = ""
            Customer_ = "0"
        Else
            Ship_code = txtConsigneeCode0_IssueCon.Value.Trim
        End If
    ...
    Protected Sub clickconsignee_Click(sender As Object, e As EventArgs)
        ...
        Dim user = (From u In db.tblParties Join br In db.tblPartyAddresses On u.PartyCode Equals br.PartyCode
                    Where u.PartyCode = lblPartyCode And br.PartyAddressCode = lblPartyAddressCode).SingleOrDefault
        txtConsigneeCode0_IssueCon.Value = user.u.PartyCode
    ...
    End Sub
    '--------------------------------------------------------------Click Search ProductCode-----------------------------------------------
    Protected Sub btnJobNo_Head_ServerClick(sender As Object, e As EventArgs)
        selectJobNoAdd()
    End Sub
    '--------------------------------------------------------Click Data ProductCode In Modal-----------------------------------------
        ...
        Try
            If e.CommandName.Equals("Selectjobnoadd") Then
                ...
                Else
                    ...
                    txtConsigneeCode0_IssueCon.Value = user.ConsigneeCode
    ...
    Protected Sub btnJobNo_Head_Edit_ServerClick(sender As Object, e As EventArgs)
        selectJobNoEdit()
    End Sub
